...
export default function InvoiceDetail() {
  ...
  // Keyboard shortcut: P = print
  useEffect(() => {
    const handler = (e) => {
      if ((e.key === "p" || e.key === "P") && !e.ctrlKey) {
        e.preventDefault();
        if (inv) printA4("invoice-detail-print");
      }
    };
  ...
  if (!inv)
    return (
      <div className="text-center py-16">
        <p className="text-ink-500">Invoice nahi mila</p>
        <Link to="/invoices" className="text-teal-600 text-sm mt-2 inline-block">
          ← Wapas invoices
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="no-print flex items-center justify-between mb-5">
        <Link
          to="/invoices"
          className="text-ink-500 hover:text-ink-900 text-sm flex items-center gap-1.5 transition-colors"
        >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => printA4("invoice-detail-print")}
      ...
      {/* Printable invoice */}
      <div id="invoice-detail-print" className="card p-8 space-y-6">
        {/* Header */}
        <div className="flex justify-between items-start">
          ...
          <div className="text-right">
            ...
            <p className="font-mono text-teal-600 font-semibold">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p className="text-xs font-display font-semibold uppercase tracking-widest text-ink-400 mb-1">
              Invoice Type
            </p>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b-2 border-ink-200">
              <th className="text-left py-2 text-xs font-display font-semibold uppercase tracking-wide text-ink-500">
                Product
              </th>
              <th className="text-center py-2 text-xs font-display font-semibold uppercase tracking-wide text-ink-500">
                Qty
              </th>
              <th className="text-right py-2 text-xs font-display font-semibold uppercase tracking-wide text-ink-500">
                Price
              </th>
              <th className="text-right py-2 text-xs font-display font-semibold uppercase tracking-wide text-ink-500">
                Disc.
        ...
        <div className="flex justify-end">
          <div className="w-64 space-y-2 text-sm">
            ...
            <div
              className={`flex justify-between font-semibold border-t border-ink-200 pt-2 ${
                inv.remaining_balance > 0 ? "text-rose-600" : "text-jade-600"
              }`}
            >
              <span>{inv.remaining_balance > 0 ? "Balance Due" : "Change"}</span>
              <span className="font-mono">
                Rs. {Math.abs(inv.remaining_balance)?.toFixed(2)}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-ink-200 pt-4 text-center">
          <p className="text-xs text-ink-400">
            Shukriya! · RxManager Pharmacy System
          </p>
        </div>
      </div>
    </div>
