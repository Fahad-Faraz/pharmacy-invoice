    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-ink-500 text-sm mt-0.5">{totalItems} products low on stock (≤10 units)</p>
        </div>
        <button
          onClick={() => printPDF("demand-print")}
      ...
      {isLoading ? (
        ...
        <div className="card p-12 text-center">
          ...
          <p className="font-display font-semibold text-ink-700">All products are well-stocked!</p>
          <p className="text-ink-400 text-sm mt-1">No items below 10 units</p>
        </div>
      ) : (
        <div id="demand-print" className="space-y-4">
          {/* Print header */}
          <div className="hidden print:block text-center mb-6">
            <h1 className="text-2xl font-bold">RxManager</h1>
            <p className="text-lg">Demand List — {new Date().toLocaleDateString("en-PK")}</p>
            <hr className="my-2"/>
          </div>
                    ...
                    <tr key={i} className="hover:bg-parchment/50 transition-colors">
                      ...
                      <td className="table-cell text-right">
                        <span className={`font-mono font-bold ${
                          p.quantity === 0 ? "text-rose-600" : p.quantity <= 5 ? "text-ember-500" : "text-ink-700"
                        }`}>
                          {p.quantity}
                        </span>
                      </td>
